...
WHERE a.relkind = 'r' AND b.nspname = '${DB_SCHEMA}' AND a.relname LIKE '${DB_WEATHER_TABLE_01}_2%'";
...
# 매달 1일마다 실행
##########################PostgreSQL################################
partition1=`psql -h ${DB_HOST} -p ${DB_PORT} -U ${DB_USER} -d ${DB_NAME} -t -c "CREATE TABLE ${DB_SCHEMA}.${DB_WEATHER_TABLE_01}_${nextPartitionYear}_${nextPartitionMonth}_1 PARTITION OF ${DB_SCHEMA}.${DB_WEATHER_TABLE} FOR VALUES FROM ('${nextPartitionDate}0100') TO ('${nextPartitionDate}1024') TABLESPACE ${DB_WEATHER_TABLE}"`;
partition2=`psql -h ${DB_HOST} -p ${DB_PORT} -U ${DB_USER} -d ${DB_NAME} -t -c "CREATE TABLE ${DB_SCHEMA}.${DB_WEATHER_TABLE_01}_${nextPartitionYear}_${nextPartitionMonth}_2 PARTITION OF ${DB_SCHEMA}.${DB_WEATHER_TABLE} FOR VALUES FROM ('${nextPartitionDate}1100') TO ('${nextPartitionDate}2024') TABLESPACE ${DB_WEATHER_TABLE}"`;
partition3=`psql -h ${DB_HOST} -p ${DB_PORT} -U ${DB_USER} -d ${DB_NAME} -t -c "CREATE TABLE ${DB_SCHEMA}.${DB_WEATHER_TABLE_01}_${nextPartitionYear}_${nextPartitionMonth}_3 PARTITION OF ${DB_SCHEMA}.${DB_WEATHER_TABLE} FOR VALUES FROM ('${nextPartitionDate}2100') TO ('${nextPartitionDate}3124') TABLESPACE ${DB_WEATHER_TABLE}"`;
...
do
	...
	#echo "Partition_start: ${nextPartitionDate}${partition_start}, Partition_end: ${nextPartitionDate}${partition_end}"
        #impala-shell -u hhiweather -k --config_file=${CONFIG_KUDU} --query "ALTER TABLE ${DB_SCHEMA}.${DB_WEATHER_TABLE_KUDU_01} ADD RANGE PARTITION '${nextPartitionDate}${partition_start}00' <= VALUES < '${nextPartitionDate}${partition_end}24'"
done
